...
from utils.monte_carlo_helpers import (
    ...
    create_martingale_histogram, # The new helper
    get_plot_path
)
import numpy as np
...
def run_european_martingale():
    ...
    #plt1 = create_bankroll_path_plot(histories, "color", wheel_type, num_spins)
    plt1 = create_bankroll_path_plot(histories, "color", wheel_type, num_spins, strategy_label="Martingale Strategy")
    
    path_plot = get_plot_path(current_folder, "european_martingale_paths.png")
    ...
    path_hist = get_plot_path(current_folder, "european_martingale_hist.png")
    ...
    # Survivors: Anyone who ended up with profit (> Start)
    winners = sum(1 for x in final_bankrolls if x > START_BANKROLL)
    
    # Bankrupt: Anyone who lost their entire bankroll (<= 0)
    # Since we allow debt, these players are technically in debt, representing total ruin
    bankrupt = sum(1 for x in final_bankrolls if x <= 0)

    # Bleeding: Anyone who lost money but still has some bankroll (0 < x <= Start)
    bleeding = sum(1 for x in final_bankrolls if 0 < x <= 1000)
    ...
    expected_loss = (num_spins * 10) * 0.027
    # New
    print(f"\nReference (If Flat Betting): ${1000 - expected_loss:.2f}")
    print("Comparison: Martingale players lost significantly more money due to huge wagers.")

if __name__ == "__main__":
    run_european_martingale()